	...
	<table class="table table-bordered table-hover" id="table1">
		...
		<tbody>
			...
			foreach ($acos as $id => $aco) {
				...
				if ($ident <= $lastIdent && $lastIdent !== null) {
		...
		?><tr class='aclmanager-ident-<?php echo $ident; ?>'><?php
			...
			$icon = $allowed ? 'fa-ban' : 'fa-arrows-alt';
			$has_estado = $allowed ? 'has-error' : 'has-success'; ?>
			<td>
				<?php echo  $this->Form->input("Perms." . str_replace("/", ":", $action) . ".{$aroAlias}:{$aro[$aroAlias]['id']}",
				array('options' => array('inherit' => __('Inherit'), 'allow' => __('Allow'), 'deny' => __('Deny')),'empty' => __('No change'), '','inputGroup' => array('prepend' => $icon), 'label' => false, 'class' => 'form-control ', 'wrap' => $has_estado, 'value' => $value)); ?>
